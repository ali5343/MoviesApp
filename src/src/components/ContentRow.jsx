...
const ContentRow = ({ title, items = [], viewAll = false, type = "normal", comingSoon = false }) => {
  ...
  const loadMoreItems = () => {
    setVisibleItemsCount((prev) => {
      const newCount = Math.min(prev + 4, items.length);
      if (newCount >= items.length) setShowViewMore(false);
      return newCount;
    });
  };

  const isGrid = isMobile && title !== "Coming Soon";
  const visibleItems = isGrid ? items.slice(0, visibleItemsCount) : items;
    ...
    <>
      <div className="py-5 relative group">
        ...
        <div className="relative">
          ...
          <div
            ref={containerRef}
            className={`${isGrid ? "grid grid-cols-2 gap-3" : "flex overflow-x-auto space-x-3 pb-4 scrollbar-hide"}`}
          >
            {visibleItems.map((item) => (
              <div
                key={item.id}
                className={`${isGrid ? "relative group/item cursor-pointer" : "flex-shrink-0 relative group/item cursor-pointer"}`}
                style={{ width: isGrid ? "100%" : type === "ranked" ? "180px" : "170px" }}
              ...
              >
                  ...
                  {comingSoon && (
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <div className="text-sm">{item.episodes || "Coming Soon"}</div>
                      </div>
                    </div>
                  )}
                ...
                <div className="p-2">
                  ...
                  {item.episodes && <p className="text-xs text-gray-400">{item.episodes}</p>}
                </div>
              </div>
            ))}
          </div>
      ...
      {selectedShow && <DetailView show={selectedShow} onClose={handleCloseDetail} />}
    </>
  );
};
